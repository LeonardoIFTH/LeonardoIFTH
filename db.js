...
const DB_NAME='hidro_db'; const DB_VERSION=3; let dbPromise=null;
function openDB(){ if(dbPromise) return dbPromise; dbPromise=new Promise((resolve,reject)=>{ const req=indexedDB.open(DB_NAME,DB_VERSION);
  req.onupgradeneeded=()=>{ const db=req.result;
    ...
    if(!db.objectStoreNames.contains('hidrometers')){ const s=db.createObjectStore('hidrometers',{keyPath:'id',autoIncrement:true}); s.createIndex('tag','tag'); s.createIndex('createdAt','createdAt'); }
    if(!db.objectStoreNames.contains('visitReports')){ const s=db.createObjectStore('visitReports',{keyPath:'id',autoIncrement:true}); s.createIndex('category','category'); s.createIndex('createdAt','createdAt'); }
  };
...
export async function clearAll(){ const db=await openDB(); return Promise.all(['clients','locations','metals','hidrometers','visitReports'].map(n=> new Promise((res,rej)=>{ const r=db.transaction(n,'readwrite').objectStore(n).clear(); r.onsuccess=()=>res(); r.onerror=()=>rej(r.error);}))); }
